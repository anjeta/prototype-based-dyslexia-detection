"""
Created on Mon May 27 16:31:54 2024
...
def save_logs(data, logs_path, logs_filename):
    ...
    with open(os.path.join(logs_path, logs_filename), 'w') as file:
        json.dump(data, file, indent=2)
        

def run(trainLoader, validLoader, testLoader, 
    ...
    if log_params.results_path is not None:
        if not os.path.exists(log_params.results_path):
            os.makedirs(log_params.results_path)
    ...
    for epoch in range(start_epoch, train_params.num_epochs):
        ...
        if log_params.results_path is not None and ((epoch+1) % train_params.save_step == 0 or (epoch+1) == train_params.num_epochs):
            if train_params.save_step != -1 and epoch != 0:
                Model.save_model(os.path.join(log_params.results_path, f"checkpoint_{epoch+1}.pt"))
            save_logs(logs, log_params.results_path, "checkpoint_logs.json")
    ...
    for key, value in dict(eval_logs).items():
        ...
        if isinstance(value, np.ndarray):
            value = value.tolist()
        logs[key].append(value)
    if log_params.results_path is not None and (epoch == train_params.num_epochs):
        if train_params.save_step != -1 and epoch != 0:
            Model.save_model(os.path.join(log_params.results_path, f"checkpoint_{epoch}.pt"))
    save_logs(logs, log_params.results_path, "checkpoint_logs.json")
...
def main():
    ...
    Model = module.Model
    params = importlib.import_module(f'model.params.{model_architecture}_params')
    ...
    fname = f"{model_architecture}_training_log.log"
    ...
    if train_params.balanced_training:
        if model_params.criterion == 'BCE_with_logits':
            target = [int(label.detach().cpu().numpy()[0]) for _, label in trainDataset]
        else:
            target = [int(label.detach().cpu().numpy()) for _, label in trainDataset]
    ...
    testLoader = DataLoader(dataset=testDataset, shuffle=False, drop_last=False, batch_size=batch_size, num_workers=data_params.num_workers)
    ...
    if pretrained_architecture is None:
        ...
        run(trainLoader, validLoader, testLoader, model, train_params, log_params, logs, logger)
        ...
        pretrain_log_params.results_path = os.path.join(results_path, "stage_0/")
        pretrain_train_params.save_step = 50
        pretrain_train_params.num_epochs = 100
        run(trainLoader, validLoader, testLoader, deepcopy(model), pretrain_train_params, pretrain_log_params, deepcopy(logs), logger)
        
        # Stage 1: Warm-up ----------------------------------------------------
        logger.info(f"{curr_time} | INFO | train_model | main | Stage 1: Warm-up")
        log_params.checkpoint_path = pretrain_log_params.results_path
        log_params.results_path = os.path.join(results_path, "stage_1/")
        ...
        run(trainLoader, validLoader, testLoader, deepcopy(model), train_params, log_params, deepcopy(logs), logger)
        ...
        log_params.results_path = os.path.join(results_path, "stages_2_3_4/")
        train_params.save_step = 100
